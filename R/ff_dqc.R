...
ff_dqc <- function(folder_path, return_values = TRUE) {
  summary_by_feature <- function(dataframe, feature) {
    ...
    if (nrow(seldf) == 1) {
      seldf <- rbind(seldf, seldf)
      type <- "static"
    }
    return(list(
      "feature" = feature,
      "type" = type,
      "mindate" = if (is.na(seldf$dates[1])) {
        NA
      } else {
        as.character(min(as.Date(seldf$dates)))
      },
      "maxdate" = if (is.na(seldf$dates[1])) {
        NA
      } else {
        as.character(max(as.Date(seldf$dates)))
      },
      "gaps" = if ((max(diff(sort(as.Date(seldf$dates)))) < 32) || (min(diff(sort(as.Date(seldf$dates)))) > 364)) {
        "no"
      } else {
        "yes"
      },
      "doubles" = if ((type == "static") || (min(diff(sort(as.Date(seldf$dates)))) > 27)) {
        "no"
      } else {
        "yes"
      },
      "npixel" = if (sd(seldf$npixel) == 0) {
        seldf$npixel[1]
      } else {
        "diff"
      },
      "xmin" = if (sd(seldf$xmin) == 0) {
        seldf$xmin[1]
      } else {
        "diff"
      },
      "xmax" = if (sd(seldf$xmax) == 0) {
        seldf$xmax[1]
      } else {
        "diff"
      },
      "ymin" = if (sd(seldf$ymin) == 0) {
        seldf$ymin[1]
      } else {
        "diff"
      },
      "ymax" = if (sd(seldf$ymax) == 0) {
        seldf$ymax[1]
      } else {
        "diff"
      },
      "resolution" = if (sd(seldf$resolution) == 0) {
        seldf$resolution[1]
      } else {
        "diff"
      },
      "crsname" = if (length(unique(seldf$crsname)) == 1) {
        seldf$crsname[1]
      } else {
        "diff"
      },
      "crscode" = if (length(unique(seldf$crscode)) == 1) {
        seldf$crscode[1]
      } else {
        "diff"
      },
      ...
      "hasNA" = if (length(unique(seldf$hasNA)) == 1) {
        seldf$hasNA[1]
      } else {
        "diff"
      }
  ...
  return(list(
    ...
    "minextent" = terra::ext(max(allvals$xmin), min(allvals$xmax), max(allvals$ymin), min(allvals$ymax))
  ))
}
